...
function [ x, y, s ] = Scopt01( A, b, c, K )
...
%     dkappa = Delta( 2*n + m + 2 ) ;
    ...
    D     = invG*invTheta ;
    r3Apo = r3 + r5/tau ;
    r2Apo = r2 + Theta*G*T*invXTilde*r4 ;
    r1Apo = r1 - A*D^( 2 )*r2Apo ;
    
    r3ApoApo = r3Apo + c'*D^( 2 )*r2Apo ;
    
    a1 = -A*D^( 2 )*c - b ;
    a2 = -b + A*D^( 2 )*c ;
    a3 = kappa/tau + c'*D^( 2 )*c ;
    
    aBar = a1/a3 ;
    aHat = a2 ;
    
    xi = r1Apo - ( r3ApoApo/a3 )*a1 ;
    
%     % 方法一
%     dy     = pinv( A*D^( 2 )*A' + aBar*aHat' )*xi ;   % 待优化

%     % 方法二
%     Aug = [ A*D^( 2 )*A' + aBar*aHat', xi ] ;
%     RR   = rref( Aug ) ;                                 % s使用行最简型求线性方程组
%     dy  = RR( :, end ) ;

%     % 方法三
%     [ LL, UU ] = lu( A*D^( 2 )*A' + aBar*aHat' ) ;        % LU 分解求线性方程组
%     dy = UU\( LL\xi ) ;
    
    % 方法四
    [ QQ, RR ] = qr( A*D^( 2 )*A' + aBar*aHat' ) ;        % QR 分解求线性方程组
    dy = RR\( QQ\xi ) ;

%     % 方法四
%     [ UU, TT ] = schur( A*D^( 2 )*A' + aBar*aHat' ) ;        % schur 分解求线性方程组
%     dy = UU'\( TT\( UU\xi ) ) ;
    
%     % 方法五
%     dy = [ A*D^( 2 )*A' + aBar*aHat' ]\xi ;             % 矩阵除法求线性方程组

%     % 方法六
%     [ R, p ] = chol( A*D^( 2 )*A' + aBar*aHat' ) ;        % Cholesky 分解求线性方程组
%     dy = R\( R'\xi ) ;
    
    dtau   = ( r3ApoApo + a2'*dy )/a3 ;
    dx     = D^( 2 )*r2Apo + D^( 2 )*A'*dy - D^( 2 )*c*dtau ;
    ds     = Theta*G*invXTilde*r4 - ( Theta*G )^( 2 )*dx ;
